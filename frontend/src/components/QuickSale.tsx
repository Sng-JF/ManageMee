...
export default function QuickSale({ menuItem, inventoryItems, onClose, onConfirm, onSaleLogged, initialQuantity = 1 }: QuickSaleProps) {
  ...
  useEffect(() => {
    ...
    setShowIngredientAdjustments(false);
    setSaleError(null);
    setAnimationStage('input');
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="absolute inset-0 bg-black/60 z-50 p-4 overflow-hidden"
      ...
      >
        <div className="mx-auto h-full max-w-md">
          <motion.div
            ...
            className="bg-white rounded-2xl h-full overflow-hidden flex flex-col"
          >
            <div className={`sticky top-0 z-10 text-white p-5 flex items-center justify-between ${
            ...
            <div className={`flex-1 min-h-0 overflow-y-auto overscroll-contain ${hiddenScrollbarClass}`}>
              <div className="p-5">
                {animationStage === 'input' && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    {showIngredientAdjustments && (
                      <div
                        className={`bg-gray-50 border-2 border-gray-300 rounded-lg p-4 mb-6 space-y-3 max-h-[40vh] overflow-y-auto overscroll-contain ${hiddenScrollbarClass}`}
                      >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex-1 text-center">
                                  <input
                                    ...
                                    className={`text-2xl font-bold w-16 text-center bg-transparent ${
                                      isExcluded
                                        ? 'text-red-600'
                                        : isModified
                                        ? 'text-orange-600'
